...
int utn_getChar(char input[], char mensaje[], char mensajeError[], int tam, int reintentos)
{
	int isOk = 1;
    char auxString[tam];
    ...
    if(input != NULL && mensaje != NULL && mensajeError != NULL && tam > 0 && reintentos >= 0)
    {
        do{
        	printf("%s", mensaje);
            if(!(getString(auxString,tam)) && !(isCharDos(auxString)))
            {
                strncpy(input,auxString,tam);
                isOk = 0;
                break;
